...
// On macOS Apple Silicon it reads the IOKit HID accelerometer directly.
// On Linux it uses the built-in microphone via arecord (--mic mode).
package main
...
func main() {
	cmd := &cobra.Command{
		...
		Long: `spank detects physical impacts on the laptop and plays audio responses.

On macOS Apple Silicon it reads the IOKit HID accelerometer (requires sudo).
On Linux use --mic to detect knocks via the built-in microphone (no sudo needed).
...
Use --mic for microphone-based detection on Linux (requires arecord / alsa-utils).`,
		Version: version,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --fast lowers the mic threshold when --mic-threshold is at its default
			if fastMode && micMode && !cmd.Flags().Changed("mic-threshold") {
				micThreshold = 0.01
			}
	...
	cmd.Flags().BoolVar(&micMode, "mic", false, "Use microphone-based knock detection (Linux only, for non-Apple hardware)")
	cmd.Flags().StringVar(&micDevice, "mic-device", "hw:0,6", "ALSA capture device for --mic mode (use 'arecord -l' to list devices)")
	cmd.Flags().Float64Var(&micThreshold, "mic-threshold", 0.02, "RMS amplitude threshold for --mic mode (0.0-1.0, lower = more sensitive)")
...
func run(ctx context.Context, tuning runtimeTuning) error {
	...
	if micMode {
		return runMic(ctx, pack, tuning)
	}
	return runAccel(ctx, pack, tuning)
}
